...
namespace three_year
{
	...
	int CJSClient::CreateSocket()
	{
		...
		int ret1 = setsockopt(m_fd, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(char));
		...
		int reuse = 1;
		int ret = setsockopt(m_fd, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse, sizeof(reuse));
		if (ret == SOCKET_ERROR)
		{
			printf("_SetReuseAddr failed, err_code=%d\n", WSAGetLastError());
		}

		struct linger so_linger1;
		socklen_t len = sizeof(so_linger1);
		ret = getsockopt(m_fd, SOL_SOCKET, SO_LINGER, (char*)&so_linger1, &len);

		struct linger so_linger;
		so_linger.l_onoff = TRUE;
		so_linger.l_linger = 10;
		ret = setsockopt(m_fd, SOL_SOCKET, SO_LINGER, (const char*)&so_linger, sizeof(struct linger));

		struct linger so_linger2;
		len = sizeof(so_linger2);
		ret = getsockopt(m_fd, SOL_SOCKET, SO_LINGER, (char*)&so_linger2, &len);

		sockaddr_in serv_addr;
		memset(&serv_addr, 0, sizeof(sockaddr_in));
		serv_addr.sin_family = AF_INET;
		serv_addr.sin_port = htons(1987);
		serv_addr.sin_addr.s_addr = inet_addr("10.1.1.167");
		if (serv_addr.sin_addr.s_addr == INADDR_NONE)
		{
			hostent* host = gethostbyname("10.1.1.167");
			if (host != NULL)
			{
	...
	int CJSClient::Connect(const char* ip, int port)
	{
		...
		if (ret < 0)
		{
			printf("Connect Server[%s]--Port[%d] fail, error[%d]\n", ip, port, WSAGetLastError());
			return ret;
		}
